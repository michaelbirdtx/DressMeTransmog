...
mainFrame.buttons.apply = CreateFrame("Button", "$parentButtonApply", mainFrame, "UIPanelButtonTemplate2")

do
    local btn = mainFrame.buttons.apply
    ...
    btn:SetScript("OnClick", function()
        ...
        for i=1, #slotOrder do
            if slots[slotOrder[i]].itemId ~= nil then
                SendChatMessage("#transmog "..slotIds[slots[slotOrder[i]].slotName].." "..slots[slotOrder[i]].itemId,"SAY")
            end
        end
    end)
end
...
do
    ...
    btn:SetPoint("TOPRIGHT", mainFrame.buttons.apply, "TOPLEFT")
    btn:SetWidth(mainFrame.buttons.apply:GetWidth())
    btn:SetText("Reset")
    btn:SetScript("OnClick", function()
...
do
    ...
    btn:HookScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("The target must be within inspection range.", 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
...
local function slot_OnEnter(self)
    ...
    if self.itemId == nil then
        ...
        if GetSettings().showShortcutsInTooltip then
            GameTooltip:AddLine(" ")
            GameTooltip:AddLine("|cff00ff00Shift + Left Click:|r create a chat link for this item.")
            GameTooltip:AddLine("|cff00ff00Right Click:|r undress this slot.")
            GameTooltip:AddLine("|cff00ff00Ctrl + Left Click:|r apply the look for this item.")
        end
    end
    GameTooltip:Show()
end
...
do
    ...
    btn:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("An approximation of this outfit in Shadowform.", 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
    ...
    list.onEnter = function(self)
        ...
        if GetSettings().showShortcutsInTooltip then
            ...
            if #ids > 1 then
                ...
                GameTooltip:AddLine("|cff00ff00Shift + Left Click:|r create a chat link for the chosen item.")
            else
                GameTooltip:AddLine("|cff00ff00Shift + Left Click:|r create a chat link for the item.")
            end
...
do
    ...
    btnTryOn:HookScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("May not be immediate if there are items in the chosen look that must be queried and cached.", 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
...
do  --------- Preview Setup
    ...
    tipFrame:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("Please leave this set to classic; modern looks are not implemented in this version.", 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
...
do  --------- Ignore UI sclaing
    ...
    checkbox:HookScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("The game's 3D rendering can break preview displays if UI scaling is set too low. Set this checkbox to ignore UI scaling.", 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
    checkbox:HookScript("OnLeave", function(self)
        GameTooltip:Hide()
